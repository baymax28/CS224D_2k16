...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    """
    ### YOUR CODE HERE
    with tf.name_scope("softmax-classifier"):
      w = tf.Variable(tf.zeros([self.config.n_features, self.config.n_classes])) # (n_features, n_classes)
      b = tf.Variable(tf.zeros([1, self.config.n_classes])) # (1, n_classes)
      scores = tf.add(tf.matmul(input_data, w), b)
