...
class Config(object):
  ...
  """
  embed_size = 50
  batch_size = 101810#67873#40724#64
  label_size = 5
  hidden_size = 100
  max_epochs = 24 
  early_stopping = 2
...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      embeddings = tf.Variable(tf.random_uniform((len(self.wv), self.config.embed_size), minval = -1, maxval = 1))
  ...
  def add_model(self, window):
    ...
    with tf.name_scope("Layer"):
      w = tf.Variable(initializer((self.config.window_size * self.config.embed_size, 
        self.config.hidden_size)))
      b1 = tf.Variable(initializer((self.config.hidden_size,)))
    with tf.name_scope("Softmax"):
      u = tf.Variable(initializer((self.config.hidden_size, self.config.label_size)))
