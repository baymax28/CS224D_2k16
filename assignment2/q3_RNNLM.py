...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      # ### YOUR CODE HERE
      # embeddings = tf.Variable(tf.random_uniform((len(self.vocab), self.config.embed_size), 
      #   minval = -1, maxval = 1))
      ...
      embeddings = tf.Variable(tf.random_uniform((len(self.vocab), self.config.embed_size), 
        minval = -1, maxval = 1))
      embed = tf.nn.embedding_lookup(embeddings, self.input_placeholder)
      l = tf.split(1, self.config.num_steps, embed)
      embed_reshaped = tf.squeeze(l, [2])
      inputs = embed_reshaped
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    Args:
      rnn_outputs: List of length num_steps, each of whose elements should be
                   a tensor of shape (batch_size, embed_size (hidden_size?)).
    Returns:
    ...
    U = tf.Variable(initializer((self.config.hidden_size, len(self.vocab))))
    b_2 = tf.Variable(initializer((len(self.vocab), )))
    outputs = []
    for op in rnn_outputs:
      outputs.append(tf.add(tf.matmul(op, U), b_2))
    ### END YOUR CODE
    return outputs
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN"):
      H = tf.get_variable("H", initializer = initializer((self.config.hidden_size, self.config.hidden_size)))
      I = tf.get_variable("I", initializer = initializer((self.config.embed_size, self.config.hidden_size)))
      b_1 = tf.get_variable("bias", initializer = initializer((self.config.hidden_size, )))
    self.initial_state = tf.zeros((self.config.batch_size, self.config.hidden_size))
    with tf.variable_scope("RNN", reuse = True):
      rnn_outputs = []
      hidden_state = tf.matmul(self.initial_state, H) + tf.matmul(inputs[0], I) + b_1
      rnn_outputs.append(hidden_state)
      for idx in range(1, inputs.get_shape()[0]):
        H = tf.get_variable("H")
        I = tf.get_variable("I")
        b_1 = tf.get_variable("bias")
        hidden_state = tf.matmul(hidden_state, H) + tf.matmul(inputs[idx], I) + b_1
        rnn_outputs.append(hidden_state)
